...
def crawl_regulator(
    ...
    while queue and pages_crawled < max_pages:
        ...
        if not allowed_by_robots(url, rp_cache):
        ...
        # If this URL is a PDF itself
        if "application/pdf" in ctype.lower() or PDF_EXT_RE.search(url):
            # Do not treat as page, handle as PDF direct
...
def handle_pdf(
    ...
    if "application/pdf" not in ctype and not PDF_EXT_RE.search(pdf_url):
        # Some servers mislabel content-type. We can peek first bytes for %PDF
        head = r.raw.read(5, decode_content=True)
        if head != b"%PDF-":
            print(f"[skip] Not a PDF by magic header {pdf_url}")
            return
        # we already consumed 5 bytes, reconstruct stream
        content = head + r.content
    else:
        content = r.content
    ...
    base = slugify(anchor_title) if anchor_title else slugify(url_path_name)
    ...
    while final_path.exists():
        ...
        final_path = out_dir / "pdf" / f"{stem}_{i}{suffix}"
        i += 1
